...
import { usePermissionStore, useAccountStore } from "@/store";
import { ROLE_ROOT, ACCESS_TOKEN_KEY } from "@/constants";
// utils
...
export function setupPermission() {
  ...
  const whiteList: any[] = [
    RouterPath.LOGIN,
    RouterPath.REGISTER,
    RouterPath.FORGOT_PASSWORD,
  ];

  router.beforeEach(async (to, from, next) => {
    NProgress.start();

    const isLoggedIn = Storage.get(ACCESS_TOKEN_KEY);
...
async function handleAuthenticatedUser(
  ...
  try {
    // 检查用户信息是否存在
    if (!accountStore.accountInfo.username) {
 ...
 * 重置用户状态并重定向到登录页
 */
async function resetUserStateAndRedirect(
  to: RouteLocationNormalized,
  next: NavigationGuardNext
) {
  try {
    await useAccountStore().resetAllState();
    redirectToLogin(to, next);
  } catch (resetError) {
...
function redirectToLogin(
  ...
  next(`${RouterPath.LOGIN}?redirect=${encodeURIComponent(redirect)}`);
}
...
export function hasAuth(
  ...
  // 超级管理员 拥有所有权限
  if (type === "button" && roleCode === ROLE_ROOT) {
    return true;
  }

  const auths = type === "button" ? perms : roleCode;

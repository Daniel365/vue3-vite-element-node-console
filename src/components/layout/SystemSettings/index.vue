<template>
  <el-drawer
    ...
    size="380"
    :title="$t('systemSetting.project')"
    :before-close="handleCloseDrawer"
    class="settings-drawer"
  >
    <div class="settings-content">
      <section class="config-section">
        <el-divider>{{ $t("systemSetting.theme") }}</el-divider>
      ...
      <!-- 界面设置 -->
      <section class="config-section">
        <el-divider>{{ $t("systemSetting.interface") }}</el-divider>

        <div class="config-item">
          <span class="text-xs">{{ $t("systemSetting.themeColor") }}</span>
          <el-color-picker
        ...
        <div class="config-item">
          <span class="text-xs">{{ $t("systemSetting.showTagsView") }}</span>
          <el-switch v-model="settingsStore.showTagsView" />
        </div>

        <div class="config-item">
          <span class="text-xs">{{ $t("systemSetting.showAppLogo") }}</span>
          <el-switch v-model="settingsStore.showAppLogo" />
        </div>

        <div class="config-item">
          <span class="text-xs">{{ $t("systemSetting.showWatermark") }}</span>
          <el-switch v-model="settingsStore.showWatermark" />
        </div>
        <div v-if="!isDark" class="config-item">
          <span class="text-xs">{{ $t("systemSetting.sidebarColorScheme") }}</span>
          <el-radio-group v-model="sidebarColor" @change="changeSidebarColor">
            <el-radio :value="SidebarColor.CLASSIC_BLUE">
              {{ $t("systemSetting.classicBlue") }}
            </el-radio>
            <el-radio :value="SidebarColor.MINIMAL_WHITE">
              {{ $t("systemSetting.minimalWhite") }}
            </el-radio>
          </el-radio-group>
        </div>
      </section>
      ...
      <!-- 布局设置 -->
      <section class="config-section hidden">
        <el-divider>{{ $t("systemSetting.layout") }}</el-divider>
      ...
      <div class="action-card">
        <div class="action-buttons">
          ...
          >
            ...
            >
              {{ copyLoading ? "复制中..." : $t("systemSetting.copyConfig") }}
            </el-button>
          </el-tooltip>
          <el-tooltip content="重置将恢复所有设置为默认值" placement="top">
            ...
            >
              {{ resetLoading ? "重置中..." : $t("systemSetting.resetConfig") }}
            </el-button>
          </el-tooltip>
        </div>
      </div>
...
import { Moon, Sunny, DocumentCopy, RefreshLeft, Check } from "@element-plus/icons-vue";
import { LayoutMode, SidebarColor, ThemeMode } from "@/enums";
import { useSettingsStore } from "@/store";
import { themeColorPresets } from "@/config";
...
const layoutOptions: LayoutOption[] = [
  { value: LayoutMode.LEFT, label: getI18nText("systemSetting.leftLayout"), className: "left" },
  { value: LayoutMode.TOP, label: getI18nText("systemSetting.topLayout"), className: "top" },
  { value: LayoutMode.MIX, label: getI18nText("systemSetting.mixLayout"), className: "mix" },
];
...
const handleCopySettings = async () => {
  try {
    ...
    // 显示成功消息
    ElMessage.success({
      message: getI18nText("systemSetting.copySuccess"),
      duration: 3000,
    });
  } catch {
...
const handleResetSettings = async () => {
  ...
  try {
    ...
    ElMessage.success(getI18nText("systemSetting.resetSuccess"));
  } catch {
